...
const MLPredictionInputSchema = z.object({
  ...
  // Weather
  rainfall_deficit_pct: z.number(),  // Changed from rainfall_deficit_percent
...
export class MLModelClient {
  ...
  async predict(rawInput: MLPredictionInput): Promise<MLPredictionOutput> {
    ...
    const input = {
      ...
      rainfall_deficit_pct: rawInput.rainfall_deficit_pct > 1
        ? rawInput.rainfall_deficit_pct / 100
        : rawInput.rainfall_deficit_pct,
